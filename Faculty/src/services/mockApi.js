...
export const getFlaggedConversations = async (filters = {}) => {
  await delay();
...
export const updateConversationStatus = async (id, newStatus, auditEntry) => {
  await delay(300);
  const conversation = mockFlaggedConversations.find(conv => conv.id === id);
  if (conversation) {
...
export const assignConversation = async (id, facultyId, auditEntry) => {
  await delay(300);
  const conversation = mockFlaggedConversations.find(conv => conv.id === id);
  if (conversation) {
...
export const addConversationReply = async (id, replyData, auditEntry) => {
  await delay(300);
  const conversation = mockFlaggedConversations.find(conv => conv.id === id);
  if (conversation) {
    const newMessage = {
...
export const addCourse = async (semesterId, courseData) => {
  await delay(300);
  const semester = mockMaterialsData.semesters.find(s => s.id === semesterId);
  if (!semester) return { success: false, error: "Semester not found" };
...
export const getQuizData = async (filters = {}) => {
  await delay();
  let data = { ...mockQuizData };
  
  if (filters.course) {
    data.quizzes = data.quizzes.filter(quiz => quiz.course === filters.course);
  }
...
export const updateOutcome = async (courseId, outcomeId, outcomeData) => {
  await delay(300);
...
export const addOutcome = async (courseId, outcomeData) => {
  await delay(300);
  const course = mockLearningOutcomesData.find(c => c.courseId === courseId);
  if (!course) return { success: false, error: "Course not found" };
...
export const deleteOutcome = async (courseId, outcomeId) => {
  await delay(300);
  const course = mockLearningOutcomesData.find(c => c.courseId === courseId);
  if (!course) return { success: false, error: "Course not found" };
...
export const updateChallenge = async (courseId, challengeId, challengeData) => {
  await delay(300);
  const course = mockLearningOutcomesData.find(c => c.courseId === courseId);
  if (!course) return { success: false, error: "Course not found" };
...
export const addChallenge = async (courseId, challengeData) => {
  await delay(300);
  const course = mockLearningOutcomesData.find(c => c.courseId === courseId);
  if (!course) return { success: false, error: "Course not found" };
...
export const deleteChallenge = async (courseId, challengeId) => {
  await delay(300);
  const course = mockLearningOutcomesData.find(c => c.courseId === courseId);
  if (!course) return { success: false, error: "Course not found" };
...
export const syncCourseData = async (courseId) => {
  await delay(800);
  const course = mockLearningOutcomesData.find(c => c.courseId === courseId);
  if (!course) return { success: false, error: "Course not found" };
